package me.xpyex.plugin.flywithfood.common.command;
...
public class FWFCmdExecutor {
    ...
    public static void onCmd(FWFSender sender, String label, String... args) {
        if (Util.checkEmpty(sender, label, args)) {
            return;
        }
        ...
        if (args.length == 0) {
            for (JsonElement o : FWFConfig.CONFIG.languages.get("HelpMsgList").getAsJsonObject().get("Start").getAsJsonArray()) {
                if (o.isJsonPrimitive() && ((JsonPrimitive) o).isString()) {
                    sender.autoSendMsg(o.getAsString());
                }
            }
            if (sender.hasPermission("fly.fly")) {
                for (JsonElement o : FWFConfig.CONFIG.languages.get("HelpMsgList").getAsJsonObject().get("Fly").getAsJsonArray()) {
                    if (o.isJsonPrimitive() && ((JsonPrimitive) o).isString()) {
            ...
            if (sender.hasPermission("fly.other")) {
                for (JsonElement o : FWFConfig.CONFIG.languages.get("HelpMsgList").getAsJsonObject().get("Other").getAsJsonArray()) {
                    if (o.isJsonPrimitive() && ((JsonPrimitive) o).isString()) {
                        sender.autoSendMsg(o.getAsString().replace("%command%", "/" + label));
                    }
                }
            ...
            if (sender.hasPermission("fly.admin")) {
                for (JsonElement o : FWFConfig.CONFIG.languages.get("HelpMsgList").getAsJsonObject().get("Admin").getAsJsonArray()) {
                    if (o.isJsonPrimitive() && ((JsonPrimitive) o).isString()) {
                        sender.autoSendMsg(o.getAsString().replace("%command%", "/" + label));
                    }
                }
            }
            for (JsonElement o : FWFConfig.CONFIG.languages.get("HelpMsgList").getAsJsonObject().get(("End")).getAsJsonArray()) {
                if (o.isJsonPrimitive() && ((JsonPrimitive) o).isString()) {
                    sender.autoSendMsg(o.getAsString().replace("%command%", "/" + label));
                }
            }
        ...
        if (CHECK_ARGS.contains(args[0].toLowerCase())) {  //如果是调整飞行模式
            ...
            FWFUser target = FWFUser.of(args[1]);
            if (target == null) {
